...
import { loadProfile, loadFollowers } from '@/lib/storage';
...
export default function FollowersPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (p) setList(await loadFollowers(p.id));
      setLoading(false);
    })();
  }, [username]);

  return (
    <ListPage title="Followers" backHref={`/u/${username}`} backLabel={`@${username}`} items={list} loading={loading} emptyText="No followers yet." />
  );
}

function ListPage({ title, backHref, backLabel, items, loading, emptyText }: {
  title: string; backHref: string; backLabel: string;
  items: Profile[]; loading: boolean; emptyText: string;
}) {
  return (
    <div style={{ minHeight: '100vh' }}>
      <div style={{
        ...
        <Link href={backHref} style={topBtn}>← {backLabel}</Link>
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: 'clamp(32px, 5vw, 64px) clamp(16px, 4vw, 48px)' }}>
        {loading ? (
          ...
          <div style={centerBox}>{emptyText}</div>
        ) : (
